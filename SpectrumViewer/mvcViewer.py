...
def window_view(filename,filesource,skyline = False,flux = True,model = True,emission=False,absorption=False,
                ae = False,other = False):
    coaddObj, zObj = driver.loadFITS(filename, filesource)
    m = M.SpecModel(coaddObj, zObj)
    v=V.windowView(filename)
    c = C.SpecController(m,v)
    c.windowRun(skyline ,flux ,model ,emission,absorption,
                ae ,other )